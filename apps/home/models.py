...
class VCCSubQuestionsAnswers(db.Model):
    ...
    answer = db.Column("answer", db.Boolean, nullable=False)
    workers_id = db.Column("workers_id", pg.ARRAY(db.Integer, dimensions=1), nullable=True)
    what = db.Column("what", db.Text, nullable=True)
    when = db.Column("when", db.TIMESTAMP, nullable=True)
    state = db.Column("state", db.Text, nullable=True)
    time_creation = db.Column("time_creation", db.TIMESTAMP, default=func.now(), nullable=False)
    time_elimination = db.Column("time_elimination", db.TIMESTAMP, nullable=True)
